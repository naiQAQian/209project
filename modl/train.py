...
def main(args):
    ...
    for epoch in range(start_epoch, epochs):
        for phase in phases: #['train', 'val'] or ['train']
            ...
            for i, (x, y, mask) in enumerate(tqdm(dataloaders[phase])):
                x, y, mask = x.to(device), y.to(device), mask.to(device)

                with torch.set_grad_enabled(phase=='train'):
                    y_pred = model(torch.tensor(x, dtype=torch.float32), mask)
            ...
            #TODO: add csv, column：sample index, mse before, mse after, ssim before, ssim after, PSNR before, PSNR after.
            #TODO: add before image and after image
